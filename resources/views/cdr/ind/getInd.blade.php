...
<div class="bg-epmLighter text-black mt-16 ml-36 flex flex-wrap justify-self-center">
    ...
    <div class="w-full">
        ...
        <table class="p-2">
            ...
            @foreach($users as $user)
                <?php $day = 0; $dayTime = 0; ?>
                <tr class="border-none">
                    <td class="border-black border-1 bg-white">{{ $user->Number }} -  {{ $user->Name  }}</td>
                    <td class="pl-3 pr-3 bg-white"><?php $here = hasContext($user,$table, 8); $eight += $here->number; $day += $here->number; $dayTime += $here->time; echo $here->number; ?></td>
                    <td class="pl-3 pr-3 bg-white"><?php $here = hasContext($user,$table, 9); $nine += $here->number; $day += $here->number; $dayTime += $here->time; echo $here->number;  ?></td>
                    <td class="pl-3 pr-3 bg-white"><?php $here = hasContext($user,$table, 10); $ten += $here->number; $day += $here->number; $dayTime += $here->time; echo $here->number;  ?></td>
                    <td class="pl-3 pr-3 bg-white"><?php $here = hasContext($user,$table, 11); $eleven += $here->number; $day += $here->number; $dayTime += $here->time; echo $here->number;  ?></td>
                    <td class="pl-3 pr-3 bg-white"><?php $here = hasContext($user,$table, 12); $twelve += $here->number; $day += $here->number; $dayTime += $here->time; echo $here->number;  ?></td>
                    <td class="pl-3 pr-3 bg-white"><?php $here = hasContext($user,$table, 13); $thirteen += $here->number; $day += $here->number; $dayTime += $here->time; echo $here->number;  ?></td>
                    <td class="pl-3 pr-3 bg-white"><?php $here = hasContext($user,$table, 14); $fourteen += $here->number; $day += $here->number; $dayTime += $here->time; echo $here->number;  ?></td>
                    <td class="pl-3 pr-3 bg-white"><?php $here = hasContext($user,$table, 15); $fifteen += $here->number; $day += $here->number; $dayTime += $here->time; echo $here->number;  ?></td>
                    <td class="pl-3 pr-3 bg-white"><?php $here = hasContext($user,$table, 16); $sixteen += $here->number; $day += $here->number; $dayTime += $here->time; echo $here->number;  ?></td>
                    <td class="pl-3 pr-3 bg-white"><?php $here = hasContext($user,$table, 17); $seventeen += $here->number; $day += $here->number; $dayTime += $here->time; echo $here->number;  ?></td>
                    <td class="pl-3 pr-3 bg-white"><?php $here = hasContext($user,$table, 18); $eighteen += $here->number; $day += $here->number; $dayTime += $here->time; echo $here->number;  ?></td>
                    <td class="pl-3 pr-3 bg-white"><?php $here = 0; for ($i = 0; $i < 7; $i++) $here += hasContext($user,$table, $i)->number; for ($i = 19; $i < 23; $i++) $here += hasContext($user,$table, $i)->number; $outOfBounds += $here; $day += $here; echo $here;  ?></td>
                    <td class="pl-3 pr-3 bg-white"> {{ $day }}</td>
                </tr>
                <?php array_push($usersToSort, ["Number" => $user->Number, "Name" => $user->Name, "Time" => $dayTime, "Got" => $day]); ?>
            @endforeach
            <tr>
        ...
        <div class="w-2/3">
            ...
            <table class="p-2">
                ...
                @foreach($users as $user)
                    ...
                    <tr class="border-none">
                        <td class="border-black border-1 bg-white">{{ $user->Number }} -  {{ $user->Name  }}</td>
                        <td class="pl-3 pr-3 bg-white"><?php $here = hasContext($user,$table, 'Monday'); $monday += $here->number; $week += $here->number; echo $here->number; ?></td>
                        <td class="pl-3 pr-3 bg-white"><?php $here = hasContext($user,$table, 'Tuesday'); $tuesday += $here->number; $week += $here->number; echo $here->number; ?></td>
                        <td class="pl-3 pr-3 bg-white"><?php $here = hasContext($user,$table, 'Wednesday'); $wednesday += $here->number; $week += $here->number; echo $here->number; ?></td>
                        <td class="pl-3 pr-3 bg-white"><?php $here = hasContext($user,$table, 'Thursday'); $thursday += $here->number; $week += $here->number; echo $here->number; ?></td>
                        <td class="pl-3 pr-3 bg-white"><?php $here = hasContext($user,$table, 'Friday'); $friday += $here->number; $week += $here->number; echo $here->number; ?></td>
                        <td class="pl-3 pr-3 bg-white"><?php $here = hasContext($user,$table, 'Saturday'); $saturday += $here->number; $week += $here->number; echo $here->number; ?></td>
                        <td class="pl-3 pr-3 bg-white"><?php $here = hasContext($user,$table, 'Sunday'); $sunday += $here->number; $week += $here->number; echo $here->number; ?></td>
    ...
    <div class="w-1/3 text-center">
        ...
        <table class="p-2">
            ...
            <?php $i = 0; $lastUserNumber = 0; $usersToSort = rank($usersToSort);?>
            @foreach($usersToSort as $user)
                @php $lastUserNumber == $user["Got"]; @endphp
                <tr class="border-none">
                    <td style="padding: 5px 10px" class="border-black border-1 bg-white">@if($lastUserNumber == $user["Got"] && $i != 0) {{ $i }} @else {{ $i += 1 }} @endif</td>
                    <td style="padding: 5px 10px" class="border-black border-1 bg-white">{{ $user["Name"] }}</td>
                    <td style="padding: 5px 10px" class="border-black border-1 bg-white">{{ $user["Got"] }}</td>
                    <td style="padding: 5px 10px" class="border-black border-1 bg-white">@if($grandTotal){{ round(($user["Got"] * 100) / $grandTotal, 0) }}@else 0 @endif %</td>
                    <td style="padding: 5px 10px" class="border-black border-1 bg-white">{{ gmdate("i:s", round($user["Time"], 0)) }}</td>
                    <td style="padding: 5px 10px" class="border-black border-1 bg-white">@if($user["Got"]){{ gmdate("i:s", round(($user["Time"] / $user["Got"]), 0)) }}@else 0 @endif</td>
                </tr>
            @endforeach
        </table>
    </div>
